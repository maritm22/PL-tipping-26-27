...
# ---------- Excel parsing ----------
BONUS_ALIASES = {
    "Lag med flest scora mål": ["lag med flest scora mål", "lag med flest scorede mål", "flest scora mål", "flest scorede mål", "flest mål", "flest mal", "mest mål", "mest mal"],
...
def bonus_state(bootstrap: dict, fixtures: list[dict], gw: int, historical_players: pd.DataFrame | None = None):
    ...
    return {
        "Lag med flest scora mål": {"leaders": top_goals, "value": v_goals, "unit": "mål"},
...
def render_participant_card(scored: dict, rank: int, bonus_state_now: dict):
    ...
    with left:
        ...
        # Compact table exactly where the user sketched it: under the stats in the left column.
        st.markdown("#### Tippet tabell")

        rows = []
        for team, predicted, actual, diff, pts in scored["detail"]:
            signed = int(actual) - int(predicted)
            if signed == 0:
                avvik = "0"
            elif signed > 0:
                avvik = f"↓ {signed}"
        ...
        if rows:
            tipped_df = pd.DataFrame(rows).sort_values("Tippet")
            st.dataframe(
                tipped_df,
                hide_index=True,
                ...
                column_config={
                    ...
                    "Tippet": st.column_config.NumberColumn("Tippet", width="small", format="%d"),
                    "Reell": st.column_config.NumberColumn("Reell", width="small", format="%d"),
            ...
            st.caption("Kunne ikke koble tabelltipset mot live-tabellen.")
