...
def render_participant_card(scored: dict, rank: int, bonus_state_now: dict):
    ...
    with left:
        ...
        for team, predicted, actual, diff, pts in scored["detail"]:
            movement = int(actual) - int(predicted)
            if movement == 0:
                avvik_tekst = "0"
            elif movement > 0:
                avvik_tekst = f"↓ {movement}"
            else:
                avvik_tekst = f"↑ {abs(movement)}"
        ...
        if detail_rows:
            tipped_df = pd.DataFrame(detail_rows).sort_values("Tippet")
            st.dataframe(
                tipped_df,
                hide_index=True,
                ...
                height=520,
                column_config={
                    "Tippet": st.column_config.NumberColumn("Tippet", width="small"),
                    "Lag": st.column_config.TextColumn("Lag", width="medium"),
                    "Faktisk": st.column_config.NumberColumn("Faktisk", width="small"),
                    "Avvik": st.column_config.TextColumn(
                        "Avvik",
                        help="↑ = laget ligger høyere enn tipset. ↓ = laget ligger lavere enn tipset.",
                        width="small",
                    ),
                    "Poeng": st.column_config.NumberColumn("Poeng", width="small"),
                },
            )
        else:
            st.caption("Ingen tabelltips kunne kobles mot live-tabellen.")
