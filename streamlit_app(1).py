...
# ---------- Excel parsing ----------
BONUS_ALIASES = {
    "Lag med flest scorede mål": ["lag med flest scorede mål", "flest scorede mål", "flest mål", "flest mal", "mest mål", "mest mal"],
...
def bonus_state(bootstrap: dict, fixtures: list[dict], gw: int, historical_players: pd.DataFrame | None = None):
    ...
    return {
        "Lag med flest scorede mål": {"leaders": top_goals, "value": v_goals, "unit": "mål"},
...
def render_prediction_table(scored: dict):
    rows = []

    for team, predicted, actual, diff, pts in scored["detail"]:
        signed_diff = int(actual) - int(predicted)

        if signed_diff == 0:
            deviation = "0"
        elif signed_diff > 0:
            deviation = f"↓ {signed_diff}"
        else:
            deviation = f"↑ {abs(signed_diff)}"

        rows.append({
            "Tippet plass": int(predicted),
            "Lag": team,
            "Reell plass": int(actual),
            "Avvik": deviation,
        })

    if not rows:
        st.warning("Kunne ikke koble tabelltipset mot den reelle Premier League-tabellen.")
        if scored.get("unmatched"):
            st.caption("Ikke gjenkjente lag: " + ", ".join(scored["unmatched"]))
        return

    df = pd.DataFrame(rows).sort_values("Tippet plass").reset_index(drop=True)
    ...
    st.dataframe(
        ...
        height=735,
        column_config={
            "Tippet plass": st.column_config.NumberColumn(
                "Tippet",
                width="small",
                format="%d",
            ),
            ...
            "Reell plass": st.column_config.NumberColumn(
                "Reell",
                width="small",
                format="%d",
            ),
            "Avvik": st.column_config.TextColumn(
                "Avvik",
                width="small",
                help="↑ = laget ligger høyere enn tipset. ↓ = laget ligger lavere enn tipset.",
            ),
        },
    )
...
def render_participant_card(scored: dict, rank: int, bonus_state_now: dict):
    ...
    # Full-width table underneath the two columns.
    st.markdown("#### 📋 Tippet tabell vs. reell tabell")
    render_prediction_table(scored)
